...
//		}
	 ...
	 * @param sendType 发送类型,s单发,m群发
	 * @param mobiles 手机号码
	 * @param params 对应模板的参数值
	 * @return
	 */
	public static String sendSms(String signId, String templateId, String sendType, String mobiles, List<String> params) {
		// 查询流程
		String wsServer = getSmsWsServer();
		String urlString = wsServer + CoreConstants.SMS_WS_SEND;

		//检查必填项
		if (StrUtils.isEmpty(wsServer) || StrUtils.isEmpty(urlString) || StrUtils.isEmpty(signId) 
				|| StrUtils.isEmpty(templateId) || StrUtils.isEmpty(sendType) || StrUtils.isEmpty(mobiles)){
			ResultWS result = new ResultWS();
			result.setRet(WSConstants.FAIL);
			result.setErrorCode(WSConstants.CODE1002);
			result.setErrorMsg(WSConstants.MSG1002);
		...
		JSONObject jsonObj = new JSONObject();
		jsonObj.put("_app_code_", PropertyUtils.getConfPropertyValue(SMS_SERVICE_CODE));
		jsonObj.put("_app_psw_", PropertyUtils.getConfPropertyValue(SMS_SERVICE_PWD));
		jsonObj.put("signId", signId);
		jsonObj.put("templateId", templateId);
		jsonObj.put("sendType", sendType);
		jsonObj.put("mobiles", mobiles);
		if (null!=params && !params.isEmpty()){
			for (int i = 0;i<params.size();i++){
				switch (i) {
                case 0:
                	jsonObj.put("param1", params.get(i));
	                break;
                case 1:
                	jsonObj.put("param2", params.get(i));
	                break;
                case 2:
                	jsonObj.put("param3", params.get(i));
	                break;
                case 3:
                	jsonObj.put("param4", params.get(i));
	                break;
                case 4:
                	jsonObj.put("param5", params.get(i));
	                break;
	...
	/**
	 * 消息接口：推送消息
	 * @param msgType 手机类型：ios | android
	 * @param msg 推送消息实体
	 * @return
	 */
	public static String pushSms(String msgType, PushAppMsgVO msg) {
		// 查询流程
		String wsServer = getSmsWsServer();
		String urlString = wsServer + CoreConstants.SMS_WS_PUSH;

		//检查必填项
		if (StrUtils.isEmpty(wsServer) || StrUtils.isEmpty(urlString) || StrUtils.isEmpty(msgType) || null==msg){
			ResultWS result = new ResultWS();
			result.setRet(WSConstants.FAIL);
			result.setErrorCode(WSConstants.CODE1002);
			result.setErrorMsg(WSConstants.MSG1002);
		...
		msg.set_app_code_(PropertyUtils.getConfPropertyValue(SMS_SERVICE_CODE));
		msg.set_app_psw_(PropertyUtils.getConfPropertyValue(SMS_SERVICE_PWD));
		msg.set_app_server_ip_(PropertyUtils.getConfPropertyValue(SMS_SERVICE_IP));
		msg.setMsgType(msgType);
		
		String jsonString = JsonUtil.toJson(msg);
		String result = sendWebServiceByBody(urlString, jsonString);
		return result;
	}
	 ...
	 * @return
	 */
	public static String getSmsWsServer(){
		String wsServer = PropertyUtils.getConfPropertyValue(SMS_SERVICE_URL);//获取配置文件的配置
		if (null==wsServer || "".equals(wsServer)) {
			//wsServer = CoreConstants.SMS_WS_SERVER;
			wsServer = "http://localhost/smsServer";//@@测试，临时写死本地
		}
		return wsServer;
	}
}
