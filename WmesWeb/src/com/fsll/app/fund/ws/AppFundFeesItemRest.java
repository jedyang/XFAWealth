 ...
 */
@RestController
@RequestMapping("/service/fundFeesItem")
public class AppFundFeesItemRest extends WmesBaseRest{
	
	@Autowired
	private AppFundFeesItemService fundFeesItemService;
	
	/**
	 * 获取基金管理费用信息， 通过body传递参数
	 * 调用示例:[地址]/service/fundFeesItem/getFundChargesInfo.r
	   {"fundId":"1679091C5A880FAF6FB5E6087EB1B2DC","langCode":"sc"}
	 */
	@RequestMapping(value = "/getFundChargesInfo")
	@ResponseBody
	public ResultWS getFundChargesInfo(HttpServletRequest request,@RequestBody String body){
		...
		List<AppFundChargesDataVO> list = fundFeesItemService.fundChargesInfo(fundId,langCode);
		if(list.isEmpty()){
			result.setRet(WSConstants.FAIL);
