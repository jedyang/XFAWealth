 ...
 * @date 2017-5-4
 */
@Service("appWebFriendService")
//@Transactional
public class AppWebFriendServiceImpl extends BaseService implements AppWebFriendService {
	/**
	 * 得到distributor信息
	 * @return
	 */
	...
	public int saveWebFriend(String fromMemberId,String toMemberId,String relationships,String applyMsg,String opType) {
		if("Add".equalsIgnoreCase(opType)){
			...
			if(!list.isEmpty()){
				...
				webFriend.setRelationships(relationships);
	...
	public List<AppIfaItemVO> findMyIfaList(String memberId,String langCode){
		...
		hql.append(" select a.id from InvestorAccount a where a.ifa.id=m.id and a.member.id=? ) ");
...
//				String lastUpdate = objs[2]==null?"":objs[2].toString();
	...
	public boolean updateWebFriend(String id,String memberId,String checkResult,String checkRemark) {
		...
		WebFriend info = (WebFriend) baseDao.get(WebFriend.class, id);
		if(null == info|| !info.getToMember().getId().equals(memberId)
				|| !"0".equals(info.getCheckResult())) return false;
		
		info.setCheckResult(checkResult);
		info.setCheckRemark(checkRemark);
	...
	public JsonPaging findMyFriendList(JsonPaging jsonPaging,String memberId,String relationships,String langCode){
		StringBuilder hql = new StringBuilder();
		hql.append("from WebFriend t where t.checkResult='1' and t.fromMember.id=? ");
		List<Object> params = new ArrayList<Object>();
		params.add(memberId);
		if(!"".equals(relationships)){
			hql.append(" and t.relationships = ? ");
			params.add(relationships);
		}
		hql.append(" order by t.lastUpdate desc ");
