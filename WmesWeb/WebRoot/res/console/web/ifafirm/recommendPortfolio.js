define(function(require) {
    ...
    mybase.controller('ifaTableCtrl', ['$scope', '$http', function($scope, $http) {
    	...
    	function getDataList(num){
    		...
    		var choice=$(".funds_choice").find(".fund_choice_active");
    		var risk="";
    		var period="";
    		$.each(choice,function(){
    			var name=$(this).attr("data-name");
    			if(name=="riskLevel"){
    				risk=$(this).attr("data-value");
    			}else if(name=="period"){
    		...
    		var url = base_root +"/console/ifa/ifaRecommendPortfolioJson.do?dateStr="+new Date().getTime(),
			data =  "rows="+ rows +"&page=" + iPAGE +"&"+searchData+"&keyword=" + keyWord+"&period="+period+"&risk="+risk,
			//控制数据是否加载完成
			is_finish = false;
			$http({
				url: url,
