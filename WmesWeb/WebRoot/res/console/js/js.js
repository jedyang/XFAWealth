...
function showform(id,title,url) {
    ...
    iframe.find( 'iframe' ).attr( 'scrolling','no' );
    $( '.tab_wrap' ).append(iframe);
    var bHeight= $('iframe[name="'+id+'"]').context.body.scrollHeight;
    
    var outerHeight= $(document).height();
    if ($('iframe[name="'+id+'"]') && $('iframe[name="'+id+'"]')[0])
        outerHeight = $('iframe[name="'+id+'"]')[0].contentWindow.outerHeight;
    var height = Math.max(bHeight, outerHeight); 
    iframe.find( 'iframe' ).attr( 'height',height+100 );
