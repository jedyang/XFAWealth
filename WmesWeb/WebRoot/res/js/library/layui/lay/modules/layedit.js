...
layui.define(['layer', 'form'], function(exports){
 ...
 // console.log("aaaaaaaaaaa");
  ...
  /*var lang=null;
	$.ajax({
		url:layui.cache.dir+"/lang/lang_en.json",
		async:false,
  ...
  ,insertInline = function(tagName, attr, range){
    ...
    if(iframeDOM.selection){ //IE 
      ...
      range.select();
    }else if(iframeDOM.getSelection()){
    	 var text = range.text || attr.text;
         if(tagName === 'a' && !text) return;
         if(text){
           elem.innerHTML = text;
         }
         var node=range.createContextualFragment($(elem).prop('outerHTML'));
  ...
  ,face = function(callback){
    ...
    var faces = function(){
      ...
      layui.each(alt, function(index, item){
        arr[item] = _root + 'images/face/'+ index + '.gif';
      });
      return arr;
    }();
