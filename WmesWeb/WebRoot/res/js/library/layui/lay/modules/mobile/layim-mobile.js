...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto','jquery','face'], function(exports){
  ...
  var listTpl = function(options){
    var nodata = {
      ...
      ,history: lang.listTpl.nohistory//"暂无任何消息"
    };
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="'+ (options.index ? '{{'+ options.index +'}}' : '{{data.type}}{{data.id}}') +'" class="layim-'+ (options.type === 'history' ? '{{data.type}}' : options.type) +'{{data.id}} {{ data.status === "offline" ? "layim-list-gray" : "" }}"><div><img src="'+base_root+'{{data.avatar}}"></div><span>{{ data.username||data.groupname||data.name||"'+lang.elemChatTpl.unknowname+'" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var comTpl = function(tpl, anim, back){
    return ['<div class="layim-panel'+ (anim ? ' layui-m-anim-left' : '') +'">'
            ,'<div style="height:20px;   background-color: #36373C;" class="lay-panel">&nbsp;&nbsp;&nbsp;&nbsp;</div>'
      ,'<div class="layim-title" style="background-color: {{d.base.chatTitleColor}};">'
        ,'<p>'
          ,(back ? '<i class="layui-icon layim-chat-back" layim-event="back">&#xe603;</i>' : '<i class="layui-icon layim-chat-back" targ="1" layim-event="back">&#xe603;</i>') 
  ...
  var elemTpl = ['<div class="layui-layim">'
    ...
    ,'<div class="layim-tab-content">'
      ,'<ul class="layim-list-top">'
        ...
        ,'</li>'
        ,'{{# }); if(!d.base.copyright){ }}'
        ,'<li layim-event="about"><i class="layui-icon">&#xe60b;</i>'+lang.listTpl.about+'<i class="layim-new" id="LAY_layimNewAbout"></i></li>'
        ,'{{# } }}'
      ,'</ul>'
    ,'</div>'
  ,'</div>'
  ,'<ul class="layui-unselect layui-layim-tab">'
    ,'<li title="'+lang.app.message+'" layim-event="tab" lay-type="message" class="layim-this"><i class="layui-icon">&#xe611;</i><span>'+lang.app.message+'</span><i class="layim-new" id="LAY_layimNewMsg"></i></li>'
    ,'<li title="'+lang.app.contacts+'" layim-event="tab" lay-type="friend"><i class="layui-icon">&#xe612;</i><span>'+lang.app.contacts+'</span><i class="layim-new" id="LAY_layimNewList"></i></li>'
    ,'<li title="'+lang.app.more+'" layim-event="tab" lay-type="more"><i class="layui-icon">&#xe670;</i><span>'+lang.app.more+'</span><i class="layim-new" id="LAY_layimNewMore"></i></li>'
  ,'</ul>'].join('');
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layim-chat-send"><input type="text" autocomplete="off"><button class="layim-send layui-disabled" layim-event="send">'+lang.elemChatTpl.send+'</button></div>'
      ,'<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'<span class="layui-icon layim-tool-image" title="'+lang.elemChatTpl.uploadpic+'" layim-event="image">&#xe60d;<input type="file" name="file" accept="image/*"></span>'
        ,'{{# }; }}'*/
  ...
  var setHistory = function(data){
    ...
    cache.sortHistory = sort(cache.history, 'historyTime');
  ...
  var viewChatlog = function(page){
    ...
    $.ajax({
        ...
        data:{fromMemberId:cache.mine.id,toMemberId:thatChat.data.id,rows:20,page:page,sort:"r.createTime",order:"desc",langCode:langCode},
  ...
  //查看更多记录
  var chatListMore = function(rows){
    var thatChat = thisChat(), chatMain = thatChat.elem.find('.layim-chat-main');
    ...
    if(rows >= MAX_ITEM){
      ...
      if(!ul.prev().hasClass('layim-chat-system')){
        ul.before('<div class="layim-chat-system clickMore"><div class="rotateLoading" style="width:20px;height:20px;border:1px solid #d2d2d2;border-radius:50%;display:none;border-bottom:0;border-right:0;"></div><span layim-event="chatLog">查看更多消息</span></div>');
      }
      first.remove();
    }
    //第一次进入
    ...
    $(document).on('touchend','.layim-chat-main',function(){
		if($('.layim-chat-main').scrollTop()==0){
			function dragMove(othis){
...
//表情代码库
  ...
  var anim = 'layui-anim-upbit', events = { 
    ...
    chat: function(othis){
      ...
      if(type === 'friend'){
        ...
        data = cache["sortHistory"][list];//(local.history || {})[index] || {};
      }
      data.name = data.name || data.username || data.groupname;
      if(type !== 'history'){
        data.type = type;
  ...
  'modules/layim/mobile/layim.css?v='+layui.v
  ,'skinlayim-mobilecss'
);