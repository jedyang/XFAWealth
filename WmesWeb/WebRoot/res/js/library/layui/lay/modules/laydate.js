...
layui.define(function(exports){
  ...
  var lang=layui.getLang();
 // console.log('laydate-lang',lang);
  ...
  Dates.check = function(){
    ...
    if(value.replace(/\s/g, '') !== ''){
      if(!exp.test(value)){
        Dates.elem[as.elemv] = '';
        Dates.msg(lang.laydate.formaterror);
        return 1;
      } else if(isvoid[0]){
        Dates.elem[as.elemv] = '';
        Dates.msg(lang.laydate.validerror);
        return 1;
      } else {
  ...
  Dates.viewDate = function(Y, M, D){
    ...
    //锁定年月
    as.year.value = Dates.ymd[0] + lang.laydate.year;
    as.month.value = Dates.digit(Dates.ymd[1] + 1) + lang.laydate.month;
  ...
  Dates.festival = function(td, md){
    ...
    switch(md){
      case '1.1':
        str = lang.laydate.newyear;
      break;
      case '3.8':
        str = lang.laydate.women;
      break;
      case '4.5':
        str = lang.laydate.qingming;
      break;
      case '5.1':
        str = lang.laydate.labor;
      break;
      case '6.1':
        str = lang.laydate.children;
      break;
      case '9.10':
        str = lang.laydate.teacher;
      break;
      case '10.1':
        str = lang.laydate.national;
      break;
    };
  ...
  Dates.viewYears = function(YY){
    ...
    Dates.each(new Array(14), function(i){
      if(i === 7) {
        str += '<li '+ (parseInt(as.year.value) === YY ? 'class="'+ as[2] +'"' : '') +' y="'+ YY +'">'+ YY +lang.laydate.year+'</li>';
      } else {
        str += '<li y="'+ (YY-7+i) +'">'+ (YY-7+i) +lang.laydate.year+'</li>';
      }
    }); 
  ...
  Dates.viewtb = (function(){
    var tr, view = [], weeks = [ lang.laydate.sun, lang.laydate.mon, lang.laydate.tues, lang.laydate.wed, lang.laydate.thur, lang.laydate.fri, lang.laydate.sat];
  ...
  Dates.view = function(elem, options){
    ...
    if(!Dates.box){
      ...
      +'<ul id="laydate_hms">'
        +'<li class="laydate_sj">'+lang.laydate.time+'</li>'
      ...
      +'<div class="laydate_btn">'
        +'<a id="laydate_clear">'+lang.laydate.clear+'</a>'
        +'<a id="laydate_today">'+lang.laydate.today+'</a>'
        +'<a id="laydate_ok">'+lang.laydate.confirm+'</a>'
      +'</div>'
  ...
  Dates.events = function(){
    ...
    log.hmss = [lang.laydate.hour, lang.laydate.minute, lang.laydate.second];
    log.hmsarr = [];
    
    //生成时分秒或警告信息
    Dates.msg = function(i, title){
      var str = '<div class="laydte_hsmtex">'+ (title || lang.laydate.tips) +'<span>×</span></div>';
      if(typeof i === 'string'){
        str += '<p>'+ i +'</p>';
        Dates.shde(S('#'+as[0]));
