...
layui.define(['layer', 'laytpl', 'upload','face'], function(exports){
	var _root=base_root+'/res/js/library/layui/';
  var v = '3.5.0 Pro';
  var $ = layui.jquery;
  ...
  var faceCodeSc=face.code_sc;
  var faceCodeEn=face.code_en;
  var faceCodeTc=face.code_tc;
  
  var lang=layui.getLang(layui.jquery);
  var langCode=layui.getLangCode();
 // console.log(lang);
  ...
  LAYIM.prototype.config = function(options){
    ...
    layui.each(Array(5), function(index){
      skin.push(_root+'css/modules/layim/skin/'+ (index+1) +'.jpg')
    });
    options = options || {};
  ...
  //设置消息已读
  LAYIM.prototype.setMessageRead=function(id){
	  return setMessageRead(id), this;
  }
  ...
  var listTpl = function(options){
    var nodata = {
      friend:lang.listTpl.nofriend// "该分组下暂无好友"
      ,group:lang.listTpl.nogroup// "暂无群组"
      ,history:lang.listTpl.nohistory// "暂无历史会话"
    };
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" class="layim-'
      + (options.type === 'history' ? '{{i}}' : options.type + '{{data.id}}') 
      +' {{ data.status === "offline" ? "layim-list-gray" : "" }}"><img src="'+base_root+'{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"'+lang.elemChatTpl.unknowname
      +'" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span>'
      +'{{# if(data.newMsgCount > 0 ){ }}'
      +'<span class="layim-msg-status layui-show layui-msg-count">{{data.newMsgCount}}</span>'
      +'{{# } }}'
      +'</li>'
    ,'{{# }); if(length === 0){ }}'
      ,'<li class="layim-null">'+ (nodata[options.type] || lang.listTpl.nodata) +'</li>'
    ,'{{# } }}'].join('');
  };
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ,'<div class="layui-layim-info">'
      ...
      ,'<div class="layui-layim-status">'
        ...
        ,'<ul class="layui-anim layim-menu-box">'
          ,'<li {{d.mine.status === "online" ? "class=layim-this" : ""}} layim-event="status" lay-type="online"><i class="layui-icon">&#xe618;</i><cite class="layui-icon layim-status-online">&#xe617;</cite>'+lang.listTpl.online+'</li>'
          ,'<li {{d.mine.status === "hide" ? "class=layim-this" : ""}} layim-event="status" lay-type="hide"><i class="layui-icon">&#xe618;</i><cite class="layui-icon layim-status-hide">&#xe60f;</cite>'+lang.listTpl.invisible+'</li>'
        ,'</ul>'
      ,'</div>'
      ,'<input class="layui-layim-remark" placeholder="'+lang.listTpl.editsign+'" value="{{ d.mine.remark||d.mine.sign||"" }}">'
    ,'</div>'
    ...
    ,'{{# } }}">'
      ...
      ,' layim-this'
      ,'{{# } }}'
      ,'" title="'+lang.listTpl.connector+'" layim-event="tab" lay-type="friend">&#xe612;</li>'
      ,'<li class="layui-icon'
      ,'{{# if(!d.base.isgroup){ }}'
      ,' layim-hide'
      ...
      ,' layim-this'
      ,'{{# } }}'
      ,'" title="'+lang.listTpl.group+'" layim-event="tab" lay-type="group">&#xe613;</li>'
      ,'<li class="layui-icon" title="'+lang.listTpl.history+'" layim-event="tab" lay-type="history">&#xe611;</li>'
      
    ,'</ul>'
    ,'<ul class="layui-unselect layim-tab-content {{# if(d.base.isfriend){ }}layui-show{{# } }} layim-list-friend">'
    ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
      ,'<li>'
        ,'<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"'+lang.listTpl.nonamegroup+'"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
    ...
    ,'{{# }); if(d.friend.length === 0){ }}'
      ,'<li><ul class="layui-layim-list layui-show"><li class="layim-null">'+lang.listTpl.noconnector+'</li></ul>'
    ,'{{# } }}'
    ,'</ul>'
    ...
    ,'<ul class="layui-unselect layui-layim-tool">'
      ,'<li class="layui-icon layim-tool-search" layim-event="search" title="'+lang.listTpl.search+'">&#xe615;</li>'
      ,'{{# if(d.base.msgbox){ }}'
      ,'<li class="layui-icon layim-tool-msgbox" layim-event="msgbox" title="'+lang.listTpl.msgbox+'">&#xe645;<span class="layui-anim"></span></li>'
      ,'{{# } }}'
      ,'{{# if(d.base.find){ }}'
      ,'<li class="layui-icon layim-tool-find" layim-event="find" title="'+lang.listTpl.find+'">&#xe608;</li>'
      ,'{{# } }}'
      ,'<li class="layui-icon layim-tool-skin" layim-event="skin" title="'+lang.listTpl.changebackgroud+'">&#xe61b;</li>'
      /*,'{{# if(!d.base.copyright){ }}'
      ,'<li class="layui-icon layim-tool-about" layim-event="about" title="'+lang.listTpl.about+'">&#xe60b;</li>'
      ,'{{# } }}'*/
    ,'</ul>'
  ...
  ,'{{# }); }}'
  ,'<li layim-event="setSkin"><cite>'+lang.elemSkinTpl.simple+'</cite></li>'
  ,'</ul>'].join('');
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ,'<div class="layui-unselect layim-chat-title">'
      ,'<div class="layim-chat-other">'
        ,'<img class="layim-{{ d.data.type }}{{ d.data.id }}" src="'+base_root+'{{ d.data.avatar }}"><span class="layim-chat-username" layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||"'+lang.elemChatTpl.unknowname+'" }} {{d.data.temporary ? "<cite>'+lang.elemChatTpl.tempsession+'</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ,'<span class="layui-icon layim-tool-face" title="'+lang.elemChatTpl.selectface+'" layim-event="face">&#xe60c;</span>'
        ,'{{# if(d.base && d.base.uploadImage){ }}'
        ,'<span class="layui-icon layim-tool-image" title="'+lang.elemChatTpl.uploadpic+'" layim-event="image">&#xe60d;<input type="file" name="file"></span>'
        ,'{{# }; }}'
        ,'{{# if(d.base && d.base.uploadFile){ }}'
        ,'<span class="layui-icon layim-tool-image" title="'+lang.elemChatTpl.senddoc+'" layim-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>'
         ,'{{# }; }}'
         ,'{{# if(d.base && d.base.isAudio){ }}'
        ,'<span class="layui-icon layim-tool-audio" title="'+lang.elemChatTpl.sendaudio+'" layim-event="media" data-type="audio">&#xe6fc;</span>'
         ,'{{# }; }}'
         ,'{{# if(d.base && d.base.isVideo){ }}'
        ,'<span class="layui-icon layim-tool-video" title="'+lang.elemChatTpl.sendvideo+'" layim-event="media" data-type="video">&#xe6ed;</span>'
         ,'{{# }; }}'
        ...
        ,'<span class="layim-tool-log" layim-event="chatLog"><i class="layui-icon">&#xe60e;</i>'+lang.elemChatTpl.chatlog+'</span>'
        ,'{{# }; }}'
      ,'</div>'
      ...
      ,'<div class="layim-chat-bottom">'
        ,'<div class="layim-chat-send">'
          ,'{{# if(!d.base.brief){ }}'
          ,'<span class="layim-send-close" layim-event="closeThisChat">'+lang.elemChatTpl.close+'</span>'
          ,'{{# } }}'
          ,'<span class="layim-send-btn" layim-event="send">'+lang.elemChatTpl.send+'</span>'
          ,'<span class="layim-send-set" layim-event="setSend" lay-type="show"><em class="layui-edge"></em></span>'
          ,'<ul class="layui-anim layim-menu-box">'
            ,'<li {{d.local.sendHotKey !== "Ctrl+Enter" ? "class=layim-this" : ""}} layim-event="setSend" lay-type="Enter"><i class="layui-icon">&#xe618;</i>'+lang.elemChatTpl.sendbyenter+'</li>'
            ,'<li {{d.local.sendHotKey === "Ctrl+Enter" ? "class=layim-this" : ""}} layim-event="setSend"  lay-type="Ctrl+Enter"><i class="layui-icon">&#xe618;</i>'+lang.elemChatTpl.sendbyctrlenter+'</li>'
          ,'</ul>'
        ,'</div>'
      ,'</div>'
    ,'</div>'
  ...
  var elemAddTpl = ['<div class="layim-add-box">'
    ,'<div class="layim-add-img"><img class="layui-circle" src="'+base_root+'{{ d.data.avatar }}"><p>{{ d.data.name||"" }}</p></div>'
    ,'<div class="layim-add-remark">'
    ,'{{# if(d.data.type === "friend" && d.type === "setGroup"){ }}'
      ,'<p>'+lang.elemAddTpl.selectgroup+'</p>'
    ...
    ,'{{# if(d.data.type === "group"){ }}'
      ,'<p>'+lang.elemAddTpl.enterggrouptips+'</p>'
    ,'{{# } if(d.type !== "setGroup"){ }}'
      ,'<textarea id="LAY_layimRemark" placeholder="'+lang.elemAddTpl.validateinfo+'" class="layui-textarea"></textarea>'
    ,'{{# } }}'
    ,'</div>'
  ,'</div>'].join('');
  ...
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
    ...
    ,'{{# if(d.mine){ }}'
      ,'<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"'+lang.elemChatTpl.unknowname+'" }}'
     ,'{{# } else { }}'
      ,'{{ d.username||"'+lang.elemChatTpl.unknowname+'" }}<i>{{ layui.data.date(d.timestamp) }}</i>'
     ,'{{# } }}'
      ,'</cite></div>'
    ,'<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>'
  ,'</li>'].join('');
  
  var elemChatList = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="'+base_root+'{{ d.data.avatar }}"><span>{{ d.data.name||"'+lang.elemChatTpl.unknowname+'" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  layui.data.content = function(content){
    ...
    .replace(/face\[([^\s\[\]]+?)\]/g, function(face){  //转义表情
      ...
      var index=-1;
      if(faceCodeSc.indexOf(alt)>=0){
    	  index=faceCodeSc.indexOf(alt);
      }else if(faceCodeEn.indexOf(alt)>=0){
    	  index=faceCodeEn.indexOf(alt);
      }else if(faceCodeTc.indexOf(alt)>=0){
    	  index=faceCodeTc.indexOf(alt);
      }
      return '<img alt="'+ index +'" title="'+ faceCodes[index] +'"  src="' + faces[index] + '">';
     // return '<img alt="'+ faceCodes.indexOf(alt) +'" title="'+ alt +'"  src="' + faces[faceCodes.indexOf(alt)] + '">';
    })
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layim-list-history').on('contextmenu', 'li', function(e){
      var othis = $(this);
      var html = '<ul data-id="'+ othis[0].id +'" data-index="'+ othis.data('index') +'"><li layim-event="menuHistory" data-type="one">'+lang.listTpl.delchat+'</li><li layim-event="menuHistory" data-type="all">'+lang.listTpl.delallchatlist+'</li></ul>';
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="'+base_root+ (cache.mine.avatar||(_root+'css/pc/layim/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title|| lang.listTpl.mintitle) +'</span>'
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    if(!data.id){
      return layer.msg(lang.elemChatTpl.illegalUser);
    }
  ...
  var setChatMin = function(newMsg){
    ...
    setChatMin.index = layer.open({
      ...
      ,resize: false
      ,area: ['182px', '50px']
      ,content: '<img id="layui-layim-min" src="'+ base_root+thatChat.avatar +'"><span>'+ thatChat.name +'</span>'
      ,success: function(layero, index){
  ...
  var popAdd = function(data, type){
    ...
    return popAdd.index = layer.open({
      ...
      ,area: '430px'
      ,title: {
        friend: lang.elemAddTpl.addfriend
        ,group: lang.elemAddTpl.joingroup
      }[data.type] || ''
      ,shade: false
      ,resize: false
      ,btn: type ? [lang.elemAddTpl.confirm, lang.elemAddTpl.cancel] : [lang.elemAddTpl.sendapply, lang.elemAddTpl.close]
  ...
  var changeChat = function(elem, del){
    ...
    var thatChat = thisChat();
    setMessageRead(thatChat.data.id);
    ...
    layero.css({
      'background-image': skin ? 'url('+ skin +')' : function(){
        return cache.base.initSkin 
          ? 'url('+ (_root+'css/modules/layim/skin/'+ cache.base.initSkin) +')'
        : 'none';
      }()
    });
  };
  ...
  var sendMessage = function(){
    var data = {
      username: cache.mine ? cache.mine.username : lang.elemChatTpl.visitor
      ,avatar: cache.mine ? cache.mine.avatar : (_root+'css/pc/layim/skin/logo.jpg')
    ...
    if(data.content.replace(/\s/g, '') !== ''){
      ...
      if(data.content.length > maxLength){
        return layer.msg(lang.elemChatTpl.msglength1+ maxLength +lang.elemChatTpl.msglength2)
      }
    ...
    if (window.Notification){
      if(Notification.permission === 'granted'){
        var notification = new Notification(data.title||'', {
          body: data.content||''
          ,icon: base_root+data.avatar||'http://tp2.sinaimg.cn/5488749285/50/5719808192/1'
        });
      }else {
  ...
  var voice = function() {
    ...
    audio.src = _root+'css/modules/layim/voice/'+ cache.base.voice;
    audio.play();
  };
  ...
  var messageNew = {}, getMessage = function(data){
    ...
    if((!layimChat && data.content) || index === -1){
      ...
      if(!data.system){
        if(cache.base.notice){
          notice({
            title: lang.elemChatTpl.msgfrom1+ data.username +lang.elemChatTpl.msgfrom2
            ,content: data.content
            ,avatar: group.avatar || data.avatar
          });
        }
        
        var html='<span class="layim-msg-status layui-show layui-msg-count">1</span>';
        var ele= $('.layim-friend'+data.id).find(".layui-msg-count");
        if(ele!=undefined && ele.length>0){
        	var msgCount=ele.text();
        	if(msgCount==undefined || msgCount=='' ){
        		ele.text("1");
        	}else{
        		ele.text(parseInt(msgCount)+1);
        	}
        }else{
        	$('.layim-friend'+data.id).append(html);
        }
        
        return setChatMin({
          name: lang.elemChatTpl.newmsg
          ,avatar: group.avatar || data.avatar
          ,anim: 6
        });
    ...
    if(thatChat.data.type + thatChat.data.id !== data.type + data.id){
      ...
      var html='<span class="layim-msg-status layui-show layui-msg-count" >1</span>';
      var ele= elem.find(".layui-msg-count");
      if(ele!=undefined && ele.length>0){
      	var msgCount=ele.text();
      	if(msgCount==undefined || msgCount=='' ){
      		ele.text("1");
      ...
      var html='<span class="layim-msg-status layui-show layui-msg-count">1</span>';
       ele= $('.layim-list-friend .layim-friend'+data.id).find(".layui-msg-count");
      if(ele!=undefined && ele.length>0){
      	var msgCount=ele.text();
      	if(msgCount==undefined || msgCount=='' ){
      		ele.text("1");
      	...
      	$('.layim-list-friend .layim-friend'+data.id).append(html);
      }
  ...
  var viewChatlog = function(){
    ...
    $.ajax({
        ...
        data:{fromMemberId:cache.mine.id,toMemberId:thatChat.data.id,rows:20,page:1,sort:"r.createTime",order:"desc",langCode:langCode},
        success:function(json){
         //  console.log("gethistory:",json);
            var data=json.list;
      ...
      if(data.type === 'friend'){
        layui.each(cache.friend, function(index, item){
          if(data.groupid == item.id){
            ...
            if(has) return layer.msg(lang.elemAddTpl.friendexists1+'['+ (data.username||'') +'] '+lang.elemAddTpl.friendexists2,{anim: 6});
        ...
        if(has) return layer.msg(lang.elemAddTpl.groupin1+'['+ (data.groupname||'') +'] '+lang.elemAddTpl.groupin2,{anim: 6});
          ...
          layui.each(item1.list, function(index, item){
            if(data.id == item.id){
              ...
              if(cache.friend[index1].list.length === 0){
                li.find('.layui-layim-list').html('<li class="layim-null">'+lang.listTpl.nofriendingroup+'</li>');
              }
              return true;
            }
          });
        ...
        layui.each(cache.group, function(index, item){
          if(data.id == item.id){
            ...
            if(cache.group.length === 0){
              listElem.html('<li class="layim-null">'+lang.listTpl.nogroup+'</li>');
            }
            return true;
          }
        });
  ...
  var chatListMore = function(){
    ...
    if(length >= MAX_ITEM){
      ...
      if(!ul.prev().hasClass('layim-chat-system')){
        ul.before('<div class="layim-chat-system"><span layim-event="chatLog">'+lang.elemChatTpl.showmorelog+'</span></div>');
      }
      if(length > MAX_ITEM){
        first.remove();
      }
  ...
  var setMessageRead=function(id){
	  $('.layim-friend'+id).find(".layui-msg-count").remove();
	  $.ajax({
		  type:'post',
		  datatype:'json',
		  url:base_root+'/front/chat/setMessageRead.do',
		  data:{fromId:id,toId:cache.mine.id},
		  success:function(json){
  ...
  //表情代码库
  var faceCodes= [lang.face.smile, lang.face.grin, lang.face.laugh, lang.face.joyful, lang.face.whimper,lang.face.nosepick, lang.face.surprise, lang.face.shy,
	               lang.face.wink, lang.face.silent, lang.face.lookdown,  lang.face.love, lang.face.sob, lang.face.chuckle,  lang.face.kiss, lang.face.ill,
	               lang.face.happy, lang.face.slight,  lang.face.righthem, lang.face.lefthem,lang.face.hush, lang.face.toasted,  lang.face.wronged, lang.face.puke,
	               lang.face.yawn,  lang.face.hug, lang.face.pout, lang.face.doubt,  lang.face.greedy, lang.face.wave,  lang.face.thinking,  lang.face.sweat,
	               lang.face.drowsy, lang.face.sleep, lang.face.money, lang.face.despair, lang.face.cool, lang.face.drool, lang.face.hum,  lang.face.clap,
	               lang.face.dizzy, lang.face.sad, lang.face.crazy, lang.face.speechless, lang.face.sly, lang.face.scold, lang.face.faneachother,  lang.face.heart,
	               lang.face.heartbroken, lang.face.pig,lang.face.panda, lang.face.rabbit, lang.face.ok, lang.face.yeah, lang.face.good, lang.face.no, lang.face.strong,
	               lang.face.beckon,lang.face.weak, lang.face.horse, lang.face.what, lang.face.embarrassed, lang.face.cloud, lang.face.gelivable, lang.face.onlookers,
	               lang.face.mighty,  lang.face.altman,  lang.face.gift,  lang.face.clock, lang.face.microphone, lang.face.candle,  lang.face.cake];
  
  //表情库
  var faces = function(){
    var alt = faceCodes, arr = {};
    layui.each(alt, function(index, item){
      //arr[item] = layui.cache.dir + 'images/face/'+ index + '.gif';
    	arr[index] = _root + 'images/face/'+ index + '.gif';
    });
    return arr;
  }();
  ...
  var anim = 'layui-anim-upbit', events = {
        ...
        if(val === ''){
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
              html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+base_root+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || lang.elemChatTpl.unknowname) +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
            }
          } else {
            html = '<li class="layim-null">'+lang.listTpl.searchnodata+'</li>';
          }
          main.html(html);
          events.tab(3);
        }
    ...
    ,msgbox: function(){
      ...
      return events.msgbox.index = layer.open({
        type: 2
        ,title: lang.listTpl.msgbox
        ,shade: false
        ,maxmin: true
        ,area: ['600px', '520px']
        ,skin: 'layui-box layui-layer-border'
        ,resize: false
        ,content: cache.base.msgbox+ '?id=' + cache.mine.id+'&lang='+langCode
      });
    }
    ...
    ,skin: function(){
      layer.open({
        type: 1
        ,title: lang.listTpl.changebackgroud
        ,shade: false
        ,area: '300px'
        ,skin: 'layui-box layui-layer-border'
        ,id: 'layui-layim-skin'
    ...
    ,chat: function(othis){
     ...
     layer.close(setChatMin.index);
      ...
      if(type === 'friend'){
        ...
        data = cache[type][index];//(local.history || {})[index] || {};
      }
      data.name = data.name || data.username || data.groupname;
      if(type !== 'history'){
        data.type = type;
          ...
          ,success: function(layero){
            ...
            post(members, function(res){
              layui.each(res.list, function(index, item){
                li += '<li data-uid="'+ item.id +'"><a href="javascript:;"><img src="'+base_root+ item.avatar +'"><cite>'+ item.username +'</cite></a></li>';
                membersCache[item.id] = item;
              });
              ul.html(li);
              
              //获取群员
              othis.find('.layim-chat-members').html(res.members||(res.list||[]).length + lang.listTpl.groupmembers);
    ...
    ,face: function(othis, e){
     ...
     // var faceCode=faceCodes();
      for(var key in faces){
        content += '<li title="'+ faceCodes[key] +'"><img src="'+ faces[key] +'"></li>';
      }
        ...
        ,success: function(res){
          if(res.code == 0){
            ...
            if(type === 'images'){
              ...
              focusInsert(thatChat.textarea[0], 'file('+ (res.data.src||'') +')['+ (res.data.name||lang.elemChatTpl.downloadfile) +']');
            }
            sendMessage();
          } else {
            layer.msg(res.msg||lang.elemChatTpl.uploadfail);
          }
        }
      });
    }
    ...
    ,media: function(othis){
      var type = othis.data('type'), text = {
        audio: lang.elemChatTpl.audio
        ,video:  lang.elemChatTpl.video
      } ,thatChat = thisChat()
      
      layer.prompt({
        title: lang.elemChatTpl.videoaddr1+ text[type] + lang.elemChatTpl.videoaddr2
        ,shade: false
        ,offset: [
    ...
    ,playAudio: function(othis){
      ...
      if(!audio.play){
        return layer.msg(lang.elemChatTpl.audiotips);
      }
      if(othis.attr('status')){   
        ...
        audio.onerror = function(){
          layer.msg(lang.elemChatTpl.playaudiotips);
        };
      } 
    }
    ...
    ,playVideo: function(othis){
      ...
      if(!video.play){
        return layer.msg(lang.elemChatTpl.videotips);
      }
      layer.close(events.playVideo.index);
      events.playVideo.index = layer.open({
        type: 1
        ,title: lang.elemChatTpl.playvideo
        ,area: ['460px', '300px']
        ,maxmin: true
        ,shade: false
    ...
    ,chatLog: function(othis){
      ...
      if(!cache.base.chatLog){
        return layer.msg(lang.elemChatTpl.nomorechathistory);
      }
      layer.close(events.chatLog.index);
      return events.chatLog.index = layer.open({
        type: 2
        ,maxmin: true
        ,title: lang.elemChatTpl.chatwithhis1+ thatChat.data.name +lang.elemChatTpl.chatwithhis2
        ,area: ['450px', '100%']
        ,shade: false
        ,offset: 'rb'
        ,skin: 'layui-box'
        ...
        ,content: cache.base.chatLog + '?to=' + thatChat.data.id + '&type=' + thatChat.data.type+'&id='+cache.mine.id
      });
    }
    ...
    ,menuHistory: function(othis, e){
      ...
      var none = '<li class="layim-null">'+lang.listTpl.nohistory+'</li>'
