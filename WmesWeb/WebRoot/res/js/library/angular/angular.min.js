 ...
 angular.module('truncate', [])
    ...
    .filter('decimal',function(){
    	return function(input,key){
    		if (isNaN(key)) return input;
    		if (key <= 0) return '';
    		return input.toFixed(key);
    	};
    })
    ...
    .filter('words', function () {
    	...
    	return function (number,suffix,decimals) { 
    		/*
		    * 参数说明：
		    * number：要格式化的数字
		    * decimals：保留几位小数
    ...
    }).filter('currencyColor', function ($sce) {//数值+颜色，数值不乘100
    	return function (num,suffix,color,decimals) {
    		var original_num=num; 
    	
    		if (!num || isNaN(num)) 
    		    return $sce.trustAsHtml('<span class="price_zero">-</span>');//modified by michael 20170210
		    ...
		    value= s.join(dec)+suffix;
		    if(color=='1' && original_num<0 || color=='2' && original_num>0)
    	    	return $sce.trustAsHtml('<span class="price_negative">'+value+'</span>')
    	    else if(color=='1' && original_num>0 || color=='2' && original_num<0)
    	    	return $sce.trustAsHtml('<span class="price_positive">'+value+'</span>')
    	    return $sce.trustAsHtml('<span class="price_zero">'+value+'</span>');
    	    
    	    
    	    
    	    
    	};
    }).filter('percentColor', function ($sce) {//百分比数值+颜色，数值乘100
        return function (num,suffix,color) {
            var original_num=num; 
            
            if (!num || isNaN(num)) 
                return $sce.trustAsHtml('<span class="price_zero">-</span>');
            ...
            var cents = num.substring(num.indexOf('.')+1,num.length);
    ...
    }).filter('lang', function ($sce) {
    	return function (input,pref) {
    		input=langMutilForJs[pref+input];
    		return input;
    	};
    }).filter('userhead', function ($sce) {
    	return function (input,gender,baseroot) {
    		var url="";
    		if(input!=undefined && input!=""){
    			if(input.indexOf("/res")==0){
	                   url=input;
    			}else if (input.indexOf("/u")==0) {
    				 url="/loadImgSrcByPath.do?filePath="+input;
    			}else {
    ...
    }).filter('emotion', function ($sce) {
    	  return function (input,base_root) {
    		  var emotionsMap;
    			emotionsMap = {};
    ...
    angular.module('wmespage', []).directive('wmespage',[function(){
        return {
            restrict: 'EA',
            template: '<div class="page-list">' +
                '<ul class="wmes_list_paging" ng-show="conf.totalItems > 0">' +
                '<li class="wmes_pagint_num" ng-class="{disabled: conf.currentPage == 1}" ng-click="prevPage()"><span>&laquo;</span></li>' +
                '<li class="wmes_pagint_num" ng-repeat="item in pageList track by $index" ng-class="{wmes_pagint_now: item == conf.currentPage, separate: item == \'...\'}" ' +
                'ng-click="changeCurrentPage(item)">' +
                '<span>{{ item }}</span>' +
                '</li>' +
                '<li class="wmes_pagint_num" ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()"><span>&raquo;</span></li>' + 
                '</ul>' +
