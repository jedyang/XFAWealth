...
function(a) {
	...
	c.use = function(a, b) {
		...
		f.type = "text/css",
		f.rel = "stylesheet";
		if(c.getPath=='')c.getPath=base_root+'/res/js/library/laydate/';
	...
	c.timeVoid = function(a, b) {
		if (c.ymd[1] + 1 == c.mins[1] && c.ymd[2] == c.mins[2]) {
			if(0 === b && a == c.mins[3] && ((Number(c.hmsin[1].value) < Number(c.mins[4])) || ((Number(c.hmsin[1].value) == Number(c.mins[4])) && (Number(c.hmsin[2].value) < Number(c.mins[5]))))){
				return 1;
			}else if (0 === b && a < c.mins[3]){
				return 1;
			}
			if(Number(c.hmsin[0].value) > Number(c.mins[3]) 
					|| ((Number(c.hmsin[0].value) == Number(c.mins[3])) 
							&& (Number(c.hmsin[1].value) > Number(c.mins[4])) 
							&& (1 === b && a >= Number(c.mins[4])))){
				return 0;
			}else{
	...
	c.events = function() {
		...
		function(a, z) {
			c.on(z, "click",
			function(a) {
				// c.hasClass(this, h[1]) || c.creation([0 | this.getAttribute("y"), 0 | this.getAttribute("m"), 0 | this.getAttribute("d")])
				c.each(b("#laydate_table .laydate_click"),function(i,n){
					c.removeClass(n,'laydate_click');
				});
				c.addClass(this, "laydate_click");
				c.hasClass(this, h[1]) || c.creation([0 | this.getAttribute("y"), 0 | this.getAttribute("m"), 0 | this.getAttribute("d")],1)
			})
		}),
		...
		c.on(h.ok, "click",
		function() {
			var date = b("#laydate_table .laydate_click")[0];
			c.valid && c.creation([0 | date.getAttribute("y"), 0 | date.getAttribute("m"), 0 | date.getAttribute("d")])
