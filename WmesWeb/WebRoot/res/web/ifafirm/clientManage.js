...
define(function(require) {
    ...
    mybase.controller('ifaTableCtrl', ['$scope', '$http', function($scope, $http) {
        ...
        $scope.currency=$("#currencyCode").val();
        $scope.decimals=$("#decimals").val();
        var bellType="";
        ...
        function getDataList(){
            ...
            if($(".client-list-tab").find(".down_active").length>0){
                ...
                element=$(".client-list-tab").find(".");
            }
            var sort="total_return_rate"//$(element).attr("sort");
            var order="desc"//$(element).attr("order");
            data="sort="+sort+"&order="+order+"&";
            var url = base_root + "/console/ifafirm/client/ifaFirmClientManageJson.do";
            data +=  "rows="+ rows +"&page=" + iPAGE +searchData+"&checkIsMyFollow=1";
            var currency=$("#txtCurrency").attr("itemcode");
            data+="&cur="+currency;
            $http({
              url: url,
              method: 'POST',
...
// }
        ...
        function getSearchData(){
           ...
           // strCondition+="&cur="+$scope.currency;
            ...
            strCondition+="&clientName="+$("#clientName").val();
        ...
        $(document).on("click",".investor-xiala input,.icon_xiala",function(){
        ...
        $(".xiala-list").on("click","li",function(){
        	$("#txtCurrency").attr("itemcode", $(this).attr("id"));
    ...
    var fixHelperModified = function(e, tr) {
        ...
        var $helper = tr.clone();
        $helper.css('background','transparent');
        $helper.find('td').css('border','0');
