...
//			,100);
		...
		$(".choose-already-bottom").height($(".screener-choose-left").height() -75);
			var	myScroll = new IScroll('.choose-already-bottom', {
	        scrollbars: true, 
			...
			$.ajax({
				...
				success : function(json) {
					...
					$.each(list, function (index, array) { //遍历json数据列	
						...
						////console.log("period_code:"+periodCode+",min_increase:"+minIncrease+",max_increase:"+maxIncrease);
		...
		function rangeDraw(periodCode,minValue,maxValue){
			...
			////console.log("period_code:"+periodCode+",minValue:"+minValue+",maxValue:"+maxValue);
			var fromValue = parseFloat(minValue)*100;
			var toValue = parseFloat(maxValue)*100;
			$('.'+periodCode).val(fromValue.toFixed(2)+','+toValue.toFixed(2));
			$('.'+periodCode).jRange({
		        from: fromValue,
		        to: toValue,
		        step: 0.01,
		...
		function rangeDraw2(period,curValue){
			...
			switch(period){
			case "trailing1Mon":
				periodCode = "return_period_code_1M";
				break;
			case "trailing3Mon":
				periodCode = "return_period_code_3M";
				break;
			case "trailing6Mon":
				periodCode = "return_period_code_6M";
				break;
			case "trailingYTD":
				periodCode = "return_period_code_YTD";
				break;
			case "trailing1Year":
				periodCode = "return_period_code_1Y";
				break;
			case "trailing3Year":
				periodCode = "return_period_code_3Y";
				break;
			case "trailing5Year":
				periodCode = "return_period_code_5Y";
				break;
			case "sinceLaunch":
				periodCode = "return_period_code_LAUNCH";
				break;
			default:
				return;
			}
			...
			var fromValue = parseFloat(arr[0])*100;
			var toValue = parseFloat(arr[1])*100;
			curValue = fromValue+','+toValue;
...
//				//console.log(arr.join(","));
		...
		//点击X号关闭面板
		$(".choose-already-bottom").on('click', '.choose-already-close', '', function (e) { 
		...
		$(".funds-screnner-rows").on('click','.choose-already-close',function(e){
			$(this).parents(".attributes-date-active").removeClass("attributes-date-active");
