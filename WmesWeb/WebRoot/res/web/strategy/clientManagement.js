...
define(function(require) {
    ...
    mybase.controller('Searchctrl', ['$scope', '$http', function($scope, $http) {
	 		var url = base_root + "/front/ifa/space/accountList.do", // url
	 			iPAGE = 1, // 第N页数据
	 			iPAGE1=1,
	    		...
	    		Sortdata = "",// 排序条件初始化 默认根据issue
															// price（增长率降序）
	    		page_bol = true;// 已完成列表总页数控制
	    		unpage_bol=true;//未完成列表总页数控制
	    		// 初始化数据
	    		$scope.dataList = '';
	    	...
	    	function GetScroll(){
    			...
    			if (scrollBol && is_finish   ){
    				var status=$(".now").attr("status");
    				if(status=="1" && unpage_bol){
    					getUnList();
    				}else{
    					if(page_bol){
    						getDataList();
	    	...
	    	function getUnList(){
	    		...
	    		var   data ="rows="+ rows +"&page=" + iPAGE1 +"&cur="+currency+"&period="+period
	    		+"&distributorId="+distributorId+"&keyWord="+keyWord;
	    		data+="&sort="+sort+"&order="+order;
	    			...
	    			is_finish = false;
	    			$http({
                     url: base_root + "/front/ifa/space/unCompleteAccountList.do",
                     method: 'POST',
                     data : data,
                     headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
                	 })
	    	...
	    	$(".listContent").on('click',".delete",function(){
	    		var id=$(this).attr("itemid");
	    		layer.confirm(langMutilForJs['global.confirm.delete'],function(){
	    			$.ajax({
	    				type:'post',
	    				datatype:'json',
	    				url:base_root+'/front/investor/deleteInvestAccount.do',
	    				data:{id:id},
			...
			$(".client-list-tab li").on("click",function(){
	    		$(".client-list-tab li").removeClass("recommend-sort-active");
