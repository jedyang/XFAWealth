...
//      		 	  initInnerHeight('mainFrame','mainFrame');
    ...
    function saveData(){
	    ...
	    if (!strategyName){$.Tips({ content: "Please input strategy name."}); return;}
	    ...
	    $.ajax({
	    	type:"POST",
	        url: base_root+"/front/strategy/info/saveStrategy.do?r="+Math.random(),
	        data: data,
...
//	            	$.Tips({ content: "Password changed successfully."});
    ...
    $("#permit_client").on("click",function(){
    	...
    	if (!$(this).is(':checked')){
    		...
    		$("#permit_client").val("1");
    		$("#permit_clients").val("ALL");
    	}
    });
    $("#permit_prospect").on("click",function(){
    	if (!$(this).is(':checked')){
    		$("#permit_prospect").val("0");
    		$("#permit_prospects").val("");
    	}else{
    		$("#permit_prospect").val("1");
    		$("#permit_prospects").val("ALL");
    	}
    });
    $("#permit_buddy").on("click",function(){
    	if (!$(this).is(':checked')){
    		...
    		$("#permit_buddy").val("1");
    		$("#permit_buddies").val("ALL");
    	}
    });
    $("#permit_colleague").on("click",function(){
    	if (!$(this).is(':checked')){
    		$("#permit_colleague").val("0");
    		$("#permit_colleagues").val("");
    	}else{
    		$("#permit_colleague").val("1");
    		$("#permit_colleagues").val("ALL");
    	}
    });
    ...
    $("#push_buddy").on("click",function(){
    	if (!$(this).is(':checked')){
    		...
    		$("#push_buddy").val("1");
    		$("#push_buddies").val("ALL");
    	}
    });
    $("#push_colleague").on("click",function(){
    	if (!$(this).is(':checked')){
    		$("#push_colleague").val("0");
    		$("#push_colleagues").val("");
    	}else{
    		$("#push_colleague").val("1");
    		$("#push_colleagues").val("ALL");
    	}
    });
