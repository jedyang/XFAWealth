define(function(require) {
	...
	require('laydate');
	$(".client-more-ico,.proposal-more-ico,.plan-detail-more-ico,.client-more-ico").on("click",function(){
	...
	$('#btnFundMixAgain').click(function(){
		saveSubscription(0);
	});
	if($('.order-table-rebalance-redemption tbody tr').length < 2){
		$('.order-table-rebalance-redemption').hide();
		$('.order-setting-rebalance-redemption').hide();
	}
	if($('.order-table-subscription-purchase tbody tr').length < 2){
		$('.order-table-subscription-purchase').hide();
		$('.order-setting-subscription-purchase').hide();
	}
	if($('.order-table-rebalance-redemption tbody tr').length < 2
			&& $('.order-table-subscription-purchase tbody tr').length < 2){
		$('.no_list_tips').show();
	}else{
		$('.no_list_tips').hide();
	}
	...
	function dataValid(){
		...
		$('.order-table-subscription-purchase-tbody tr:gt(0)').each(function(){
			...
			var investorId = $(this).find('.order-table-select-account option:selected').data('investorId');
			if(investorId){
	...
	function submitData(confirmation){
		...
		$('.order-table-subscription-purchase-tbody tr:gt(0)').each(function(){
			var authorizedMap = {};
			var authorized = $(this).find('.order-table-select-account option:selected').data('authorized');
			if(authorized == '1'){
		...
		// IFA
		var ifaId = $('.order-table-select-account').eq(0).find('option:selected').data('ifaId');
		$('.order-table-rebalance-redemption tbody tr:gt(0)').each(function(){
			var checkAccount = {};
			var investorId = $(this).find('.td-investor').data('investorId');
			checkAccount['account'] = investorId+'';
			checkAccounts.push(checkAccount);
		});
		var checkAccounts = encodeURI(JSON.stringify(checkAccounts));
		...
		var url = base_root + '/front/tradeStep/saveOrderPlanCommission.do?d='+new Date().getTime();
		$.ajax({
			type:'post',
			url:url,
			data:{
			...
			success:function(result){
				if(result.flag){
					if(result.memberType != null && result.memberType == 1){
					// investor
	...
	function saveSubscription(ifEdit){
		var subscriptionDatas = [];
		$('.order-table-subscription-purchase-tbody tr:gt(0)').each(function(){
			var subscriptionData = {};
			var planProduct = $(this).data('planProduct');
		...
		$.ajax({
			...
			success:function(result){
				window.location.href = base_root + '/front/tradeMain/orderPlanSendRedirect.do?planId='+ getUrlParam('planId')+'&ifEdit='+ifEdit;
			},error:function(){
	...
	/*$(".rebalance-amount-input").on('focus',function(){
	...
	$('.table-product-tran-rate').on('input',function(){
		var rate = $(this).val();
 		var amount = $(this).closest('tr').find('.rebalance-amount-input').val();
	...
	function orderTotalFee(){
		...
		$('.order-table-subscription-purchase-tbody .product-tran-fee').each(function(){
			var amount = $(this).closest('tr').find('.rebalance-amount-input').val();
			var feeRate = $(this).closest('tr').find('.table-product-tran-rate ').val();
			var tranFee = 0.00;
			if(amount && feeRate){
				amount = amount.trim().replace(/,/g,'');
				tranFee = Number(amount)*Number(feeRate)/100;
			}
			$(this).text(formatCurrency(tranFee));
			if(tranFee){
				totalTranFee = Number(totalTranFee) + Number(tranFee);
			}
		});
		// trasnsaction amount
		$('.order-table-subscription-purchase-tbody tr:gt(0)').each(function(){
	...
	function totalAmtAdjust(){
		var totalAmt = '';
		if('1' == $('#aipState').attr('data-state')){
			totalAmt = $('#txtAipAmount').val();
			$('#txtTotalInvestmentAmt').val(totalAmt);
			if(totalAmt){
	...
	function saveAip(){
    	...
    	aipEndTotalAmount = aipEndTotalAmount.replace('-','');
