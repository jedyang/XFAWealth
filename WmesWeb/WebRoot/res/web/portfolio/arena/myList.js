...
define(function(require) {
    ...
    mybase.controller('ifaTableCtrl', ['$scope', '$http', function($scope, $http) {
    	...
    	function getDataList(num){
    		...
    		//var url = base_root + "/front/portfolio/arena/myListJson.do",
    		var url = base_root + "/front/portfolio/arena/getMyArenas.do?dateStr="+new Date().getTime(),
    		data =  "rows="+ rows +"&page=" + iPAGE +"&"+searchData+"&isPublic=" + $scope.isPublic+"&status=" + $scope.status+"&keyWord="+keyWord;
			//控制数据是否加载完成
			var is_finish = false;
			$http({
				url: url,
...
//    					//console.log(sumPage)
    	...
    	function searchList(searchLi){
    		...
    		if("riskLevel"==dataName){
    			$("#riskLevel").val(dataValue);
    		}

    		if("totalReturn"==dataName){
    			$("#totalReturn").val(dataValue);
    		}
        ...
        $(".myProposalList").on("click",".delete",function(){
        	...
        	if(!!itemId){
        		layer.confirm(langMutilForJs['global.confirm.delete'], {
        			...
        			$.ajax({
        				url:base_root+"/front/portfolio/arena/delPortfolio.do",
        				data:{"itemId":itemId},
        				type:"post",
        				dataType:"json",
    ...
    /*$(".myProposalList").on("click",".view",function(){
    	itemId = $(this).attr("itemId");//detail
    	window.location.href = base_root+"/front/portfolio/arena/createPortfolioTwo.do?edit=1&portfolioId="+itemId;
    });*/
    ...
    * 
    */ 
	$(".myProposalList").on("click",".visitorsBtn",function(){
			...
			$http({
              ...
              headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
         		 ...
         		 if(response.total<1){
         		 	...
         		 	$('.visitedconlayer').css('height',document.body.scrollHeight);
    ...
    */ 
	$(".myProposalList").on("click",".down",function(){
  	...
  	function setOverhead(itemId, type){
  		if(!!itemId && !!type){
 			$.ajax({
 				url:base_root+"/front/portfolio/arena/setOverhead.do",
