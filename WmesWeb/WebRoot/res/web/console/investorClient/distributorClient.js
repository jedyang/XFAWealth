...
define(function(require) {
    ...
    mybase.controller('Searchctrl', ['$scope', '$http', function($scope, $http) {
	 		var url = base_root + "/console/investorClient/distributorClientJson.do", // url
	 			iPAGE = 1, // 第N页数据
	 			iPAGE2= 1,
...
//	    		Sortdata = "",// 排序条件初始化 默认根据issue
	    	...
	    	function getDataList(){
	    		...
	    		var distributorId="";
				 ...
				 $(".conservative-choice li").each(function(){
					 if( $(this).hasClass('conservative-choice-active')){
						 ...
						 }else if($(this).attr("data-name")=='ifafirm'){
							 ifa=$(this).attr("data-value");
						 }else if($(this).attr("data-name")=='status'){
	    		...
	    		var data ="rows="+ rows +"&page=" + iPAGE + "&in_use="+in_use+"&inApproval="+inApproval+"&cancellation="+cancellation+"&cur="+currency+"&period="+period
	    		            +"&distributorId="+distributorId+"&keyWord="+keyWord+"&ifa="+ifa;
	    		data += "&sort="+sort+"&order="+order+"&status="+status;
...
// //console.log("list is null");
	    	...
	    	function getUnList(){
    			...
    			is_finish = false;
    			$http({
                 url: base_root + "/console/investorClient/unCompleteAccountList.do",
                 method: 'POST',
                 data : data,
                 headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
            	 })
  ...
  // var isLoaded=false;
    ...
    $("#btnAdd").on("click",function(){
    	//alert("xx");
    	window.open(base_root +"/console/investor/account/subInvAcc.do?r="+Math.random());
    });
