...
define(function(require) {
	...
	mybase.controller('ifaSpaceCtrl', ['$scope', '$http', function($scope, $http) {
		...
		function getQuestionList(){
			var url = base_root + "/front/community/space/getInvQuestionList.do",
		...
		function getDynamic(){
			...
			$http({
				...
				.success(function(response) {
					...
					if(response.rows.length==0 && $scope.dynamicPage==1){
						$('.dynamic-tips').show();
					}
					var totalPage=Math.ceil(response.total/rows);
	...
	$('.wmes-community-news-qa-flex').on('click',function(){
	...
	$(".character-button-close,.character-close-ico").on('click',function(){
		$("#askQuestion").toggleClass("ifa-space-popup")
	})
    ...
    //跳转页面
      ...
      $('#addFriend').click(function(){
			var memberId =getQueryString("id"); //$('#memberName').attr('memberid');
			var loginMemberId =_curMemberId;
			if(memberId!=''&&loginMemberId!=''&&memberId==loginMemberId){
				layer.alert(langMutilForJs['ifaspace.info.cannotAddYourself'], { title:langMutilForJs['layer.alert'], icon: 0,btn: ['OK']  });
				return;
			}
			if(memberId!=''&&(loginMemberId==''||loginMemberId==undefined)){
				layer.alert(langMutilForJs['ifaspace.info.loginToAddFriends'], { title:langMutilForJs['layer.alert'], icon: 0 ,btn: ['OK'] });
				return;
			}
			layer.confirm(langMutilForJs['ifaspace.info.sureToAddFriends'], { title:langMutilForJs['layer.alert'], icon: 3 ,btn: ['OK', 'Cancel'] },function () { 
				 $.ajax({
					type : 'post',
					datatype : 'json',
					...
					success : function(json) {
						...
						if(result==true||result=='true'){
							layer.msg(langMutilForJs['ifaspace.info.waitToAddFriends'], { title:langMutilForJs['layer.alert']  }, function () {  });
