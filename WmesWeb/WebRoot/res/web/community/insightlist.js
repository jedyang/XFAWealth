define(function(require) {
    ...
    mybase.controller('ifaTableCtrl', ['$scope', '$http', function($scope, $http) {
    	...
    	function getDataList(){
    		...
    		var page_bol;
    		var url = base_root + "/front/community/info/insightListJson.do",
    			data =  'rows='+ rows +'&page=' + iPAGE +'&id='+memberId+'&keywords=' + keywords;
    			$http({
                  url: url,
                  method: 'POST',
                  data : data,
    			...
    			.success(function(response) { 
					...
					if(response.rows.length > 0) {
						...
						$.each(response.rows,function(i,n){ 
							var islike = response.rows[i].isLike;
							var isUnlike = response.rows[i].isUnlike;
							if(islike=='1'&&isUnlike=='0')
								response.rows[i].likecss = 'comment-topper-niceed';
							else if(islike=='0'&&isUnlike=='1')
								response.rows[i].unlikecss = 'comment-topper-caied';
						});
	...
	//点赞
	  $(document).on("click",".wmes-community-news-message-right-nice",function(){
	    	...
	    	$.ajax({
	    		...
	    		data:{id:topicid,type:type,isCancel:isCancel,behavior:'like'},
	    		success:function(json){
	    			//console.log(json);
	    		}
	    	...
	    	var caielement = $(this).parent().prev().find('.wmes-community-news-message-right-cai');
	    	if(caielement.hasClass("comment-topper-caied")){
	    		caielement.toggleClass("comment-topper-caied");
	    		caielement.next().text(Number(caielement.next().text())-1);
	    	}
	  ...
	  $(document).on("click",".wmes-community-news-message-right-cai",function(){ 
		  ...
		  $(this).toggleClass("comment-topper-caied");
	    	...
	    	$.ajax({
	    		...
	    		datatype:"json",
	    		url:base_root+'/front/community/info/saveBehavior.do',
	    		data:{id:topicid,type:type,isCancel:isCancel,behavior:'unlike'},
	    		success:function(json){
	    			//console.log(json);
	    		}
	    		
	    	});
	    	var niceelement = $(this).parent().next().find('.wmes-community-news-message-right-nice');
	    	if(niceelement.hasClass("comment-topper-niceed")){
	    		niceelement.toggleClass("comment-topper-niceed");
	    		niceelement.next().text(Number(niceelement.next().text())-1);
	    	}
	  });
