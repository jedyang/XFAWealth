...
public class FundHouseServiceImpl extends BaseService implements FundHouseService {
	...
	public  FundHouseDataVO saveOrUpdate(FundHouseDataVO fundHouse){
		if(null == fundHouse.getId() || "".equals(fundHouse.getId())){
			fundHouse.setId(null);
			fundHouse.setCreateTime(new Date());
			fundHouse = (FundHouse)baseDao.create(fundHouse);
		}else{
			fundHouse = (FundHouse)baseDao.update(fundHouse);
		}
		return fundHouse;
	}*/
	...
	public FundHouseDataVO findById(String id){
		...
		if(null != fundHouse.getLogoUrl() && !fundHouse.getLogoUrl().equals("")) {
			result.setLogoUrl(fundHouse.getLogoUrl());
		}
		return result;
	}
