...
public class FrontMenuServiceImpl  extends BaseService implements FrontMenuService {
	...
	public List<MemberMenu> getFrontMenus(String roleCode) {
		...
		List<MemberMenu> menus = this.baseDao.find(hql.toString(), params.toArray(), false);
		if(menus != null && !menus.isEmpty()){
			target = new ArrayList<MemberMenu>();
			target.addAll(menus);
			getValidChilds(menus, target, roleCode);
		}
		return target;
	}

	private void getValidChilds(List<MemberMenu> menus, List<MemberMenu> target, String roleCode) {
		if(menus != null && !menus.isEmpty()){
			for (MemberMenu menu : menus) {
				StringBuffer hql = new StringBuffer(""
						+ " SELECT"
						+ " m"
						+ " FROM"
				...
				List<MemberMenu> childMenus = this.baseDao.find(hql.toString(), params.toArray(), false);
				if(childMenus != null && !childMenus.isEmpty()){
					target.addAll(childMenus);
					getValidChilds(childMenus, target, roleCode);
				}
			}
		}
	}
