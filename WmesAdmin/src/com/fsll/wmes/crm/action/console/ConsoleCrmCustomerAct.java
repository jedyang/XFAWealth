...
//			vo.setReturnRate(info.getReturnRate());
	 ...
	 * @date 2017-6-09
	 */
	@RequestMapping(value = "/console/customer/firmCustomerDetail")
	public String firmCustomerDetail(HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		this.isMobileDevice(request, model);
		String id = request.getParameter("id");
		String memberType = request.getParameter("memberType");
		model.put("id", id);
		if (CommonConstantsWeb.MEMBERBASE_SUB_MEMBERTYPE_INDIVIDUAL == Integer.parseInt(memberType)) {//11
			MemberIndividual individual = memberManageService.findIndividualById(id);
			if(individual != null){
				IndividualVO vo = new IndividualVO(individual);
				model.put("individualvo", vo);
				return "console/ifafirm/firm_customer_info";
			}
		}
		return "";
	}
	 ...
	 * @date 2017-6-09
	 */
	@RequestMapping(value = "/console/customer/disCustomerDetail")
	public String disCustomerDetail(HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		this.isMobileDevice(request, model);
		String id = request.getParameter("id");
		String memberType = request.getParameter("memberType");
		model.put("id", id);
		if (CommonConstantsWeb.MEMBERBASE_SUB_MEMBERTYPE_INDIVIDUAL == Integer.parseInt(memberType)) {//11
			...
			if(individual != null){
				IndividualVO vo = new IndividualVO(individual);
				model.put("individualvo", vo);
				return "console/distributor/dis_customer_info";
			}
		}
		return "";
	}
