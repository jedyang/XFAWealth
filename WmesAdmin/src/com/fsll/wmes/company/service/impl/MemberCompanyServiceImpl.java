...
public class MemberCompanyServiceImpl extends BaseService implements MemberCompanyService {
	...
	public List<MemberCompany> findByMemberCompany(MemberCompany info) {
		String hql = " FROM MemberCompany mc WHERE mc.member.id is not null AND mc.company.id is not null ";
	 ...
	 * @date 20170322
	 * */
	public String findOperatingCompany(String companyIfaFirmId){
		StringBuffer sb = new StringBuffer("from MemberCompanyIfafirm t where t.ifafirm.id=? ");
		List list = this.baseDao.find(sb.toString(),new String[]{companyIfaFirmId},false);
		if(null==list || list.isEmpty())return null;
		MemberCompanyIfafirm memberComanyIfaFirm = (MemberCompanyIfafirm)list.get(0);
		CompanyInfo operatingCompany = memberComanyIfaFirm.getCompany();
		return operatingCompany.getId();
	}		
	...
	public void deleteByMember(String memberId) {
		...
		base.setId(memberId);
		info.setMember(base);
		List<MemberCompany> list = findByMemberCompany(info);
		if (null!=list && !list.isEmpty()){
			for (MemberCompany c: list){
				deleteById(c.getId());
			}
		}
	}
