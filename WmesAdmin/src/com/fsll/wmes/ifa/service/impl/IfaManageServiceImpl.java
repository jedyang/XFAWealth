...
//		hql +=" lang on ifm.ifafirm.id =lang.id ";
		...
		"where ifm.checkStatus=1 and ifm.isValid=1 and ifm.ifafirm.id =?";
		String a= "8080804056a201d90156b01c3aeb0018";
		parms.add(a);
		hql+=" )";*/
		...
		if(StringUtils.isNoneBlank(ifaVo.getCompanyIfafirm()) || StringUtils.isNoneBlank(ifaVo.getIfafirmStatus())){
			hql+=" and r.id in (select ifm.ifa.id from  MemberIfaIfafirm ifm,"+getLangString("MemberIfafirm", langCode)+" lang " +
				"where ifm.ifafirm.id =lang.id and ifm.checkStatus=1 and ifm.isValid=1";
		...
		while (it.hasNext()) {
			...
			List<MemberIfaIfafirm> firms = findIfaIfaFirm(ifa.getId(), "1", "1" ,true, langCode);
	...
	public JsonPaging findIfaByifafirmId(JsonPaging jsonPaging,
		...
		if (null != ifaVo && StringUtils.isNoneBlank(ifaVo.getKeyword())) {
			//hql += " and (b.nickName like ? or b.loginCode like ? or r.nameChn like ?)";
			hql += " and (r.firstName like ? or r.lastName like ? or r.nameChn like ? or b.loginCode like ? )";
			parms.add("%"+ifaVo.getKeyword()+"%");
			parms.add("%"+ifaVo.getKeyword()+"%");
			parms.add("%"+ifaVo.getKeyword()+"%");
			parms.add("%"+ifaVo.getKeyword()+"%");
		...
		while (it.hasNext()) {
			...
			if (null!=firms && !firms.isEmpty()){
				...
				if (null!=firm && null!=firm.getIfafirm()){
					...
					vo.setIfafirmStatus(firm.getCheckStatus());
					vo.setIfafirmId(firm.getIfafirm().getId());
					vo.setEmail(firm.getIfa().getMember().getEmail());
					if(langCode.equalsIgnoreCase("en")){
						String firstName = firm.getIfa().getFirstName();
						String lastName = firm.getIfa().getLastName();
						String name = firstName + " " + lastName;
						vo.setNameChn(name);
					}else{
						vo.setNameChn(firm.getIfa().getNameChn());
					}
					
					if(firm.getIfa().getPrimaryResidence() != null) {
						vo.setPrimaryResidence(firm.getIfa().getPrimaryResidence().getCountryName(langCode));
					}else {
						vo.setPrimaryResidence("");
					}
				}
			}
