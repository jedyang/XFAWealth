...
public class MemberGroupServiceImpl extends BaseService implements MemberGroupService {
	...
	private List<MemberGroupVO> packageMemberGroupVO(String langCode,
		...
		for (MemberGroup memberGroup : groups) {
			...
			Integer count = getGroupMemberCount(vo.getId());
	...
	public Map<String, Object> addMember(String ids, String groupIds) {
		Boolean flag = false;
		if (StringUtils.isNotBlank(ids) && StringUtils.isNotBlank(groupIds)) {
			ids = StrUtils.reHeavy(ids);
			String[] objs = groupIds.indexOf(",") > -1 ? groupIds.split(",") : new String[]{groupIds}; 
			for (String groupId : objs) {
	...
	public JsonPaging getClients(JsonPaging jsonPaging, String keyWord, String langCode) {
		StringBuffer hql = new StringBuffer(" "
				...
				+ " WHERE a.openStatus=3 AND a.subFlag=0 AND a.isValid=1");
		List<Object> params = new ArrayList<Object>();
		if (StringUtils.isNotBlank(keyWord)) {
			hql.append(" AND"
					+ "("
	...
	public Integer getGroupMemberCount(String groupId){
		Integer count = 0;
		if (StringUtils.isNotBlank(groupId)) {
			StringBuffer hql = new StringBuffer(" FROM MemberGroupDetail d WHERE d.group.id=?");
			List<Object> params = new ArrayList<Object>();
			params.add(groupId);
			List<MemberGroupDetail> details = this.baseDao.find(hql.toString(), params.toArray(), false);
			if(details != null){
				count = details.size();
			}
		}
