...
public class MemberManageServiceImplForConsole extends BaseService implements MemberManageServiceForConsole {
	...
	public Map<String, Object> findMemberGroupInfo(String memberIds,
		...
		if (StringUtils.isNotBlank(memberIds)) {
			memberIds = StrUtils.reHeavy(memberIds);
			String[] objs = memberIds.indexOf(",") > -1 ? memberIds.split(",") : new String[]{memberIds};
			for (String memberId : objs) {
			...
			clientNames = clientNames != "" ? clientNames.substring(0, clientNames.length() -1) : null;
			ids = ids != "" ? ids.substring(0, ids.length() -1) : null;
			if(objs.length == 1){
				myGroups = memberGroupService.getGroupsByMember(objs[0]);
			}
		}
